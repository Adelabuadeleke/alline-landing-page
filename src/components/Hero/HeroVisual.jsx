...
export const SVGVersion = forwardRef(function SVGVersion(props, ref) {
  return (
    <svg ref={ref} className={styles.svgLogo} viewBox="0 0 520 429"
      fill="none" xmlns="http://www.w3.org/2000/svg"
      role="img" aria-label="Alline logo – architectural line drawing">
      <path className={styles.logoPathOuter} data-logo-path="outer"
      ...
      <path className={styles.logoPathInner} data-logo-path="inner"
...
export const WireframeVersion = forwardRef(function WireframeVersion(props, ref) {
  return (
    <div ref={ref} className={styles.wireframeContainer}>
      <svg viewBox="0 0 600 460" fill="none" xmlns="http://www.w3.org/2000/svg"
        className={styles.placeholderSvg} aria-label="Architectural wireframe building">
        <text x="300" y="230" textAnchor="middle" fontFamily="Inter" fontSize="13" fill="#a0a09c">
...
export const RenderedVersion = forwardRef(function RenderedVersion(props, ref) {
  ...
  React.useEffect(() => {
    const el = containerRef.current; const overlay = overlayRef.current
    if (!el || !overlay) return
    const onMove  = (e) => { const r = el.getBoundingClientRect(); overlay.style.clipPath = `circle(28% at ${((e.clientX-r.left)/r.width*100).toFixed(1)}% ${((e.clientY-r.top)/r.height*100).toFixed(1)}%)` }
    const onLeave = () => { overlay.style.clipPath = 'circle(0% at 50% 50%)' }
    el.addEventListener('mousemove', onMove); el.addEventListener('mouseleave', onLeave)
    return () => { el.removeEventListener('mousemove', onMove); el.removeEventListener('mouseleave', onLeave) }
  }, [])
  return (
    <div ref={(n) => { containerRef.current = n; if (typeof ref === 'function') ref(n); else if (ref) ref.current = n }}
      className={styles.renderedContainer}>
      <div className={styles.renderedBase}>
        <svg viewBox="0 0 600 460" fill="none" className={styles.placeholderSvg} aria-hidden="true">
          <text x="300" y="230" textAnchor="middle" fontFamily="Inter" fontSize="13" fill="#a0a09c">State 3 – hover to reveal</text>
        </svg>
      </div>
      <div ref={overlayRef} className={styles.renderedOverlay} aria-hidden="true">
        <div className={styles.renderedPlaceholder}>Rendered photo goes here</div>
      </div>
    </div>
  )
})
 ...
 * ─────────────────────────────────────────────────────────
 * Props:
 *   mode    — 'svg' | 'wireframe' | 'rendered'
 *   svgRef  — ref forwarded to SVGVersion for GSAP
 *   svgDone — boolean from Hero.jsx; when true the SVG
 *             sequence is complete, HeroCanvas mounts with
 *             startOrbit=true (one-shot overhead→isometric
 *             camera move, then holds still forever).
 */
export default function HeroVisual({ mode = 'svg', svgRef, svgDone = false }) {
  const canvasWrapRef = React.useRef(null)

  /* Fade the 3D canvas wrapper in when it mounts */
  useEffect(() => {
    if (!svgDone || !canvasWrapRef.current) return
    gsap.fromTo(
  ...
  switch (mode) {
    case 'wireframe': return <WireframeVersion />
    case 'rendered':  return <RenderedVersion />
    case 'svg':
    default:
      if (svgDone) {
        return (
          <div ref={canvasWrapRef} style={{ opacity: 0, width: '100%' }}>
            <HeroCanvas startOrbit />
          </div>
        )
      }
